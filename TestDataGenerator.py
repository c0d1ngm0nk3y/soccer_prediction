...
class TestDataGenerator(object):
    ...
    def genererateFromGameDay(self, game_day, game_table, game_table_trend):
        ...
        for game in game_day:
            ...
            pos_home_trend = game_table_trend.get_position(ht)
            x_pos_home = self.get_input_for_position(pos_home)
            x_pos_home_trend = self.get_input_for_position(pos_home_trend)

            at = game.get_away_team()
            ap = game.get_away_points()
            pos_away = game_table.get_position(at)
            pos_away_trend = game_table_trend.get_position(at)
            x_pos_away = self.get_input_for_position(pos_away)
            x_pos_away_trend = self.get_input_for_position(pos_away_trend)
